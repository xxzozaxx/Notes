...
;; Remove all nodes which satisfy fn condition
(define (actRemove fn act)
  (cond [(false? act) false]
        [else (if (fn act)
                  (join (actRemove fn (node-l act))
                        (actRemove fn (node-r act)))
                  (make-node (node-id act)
                             (node-name act)
                             (node-bal act)
                             (actRemove fn (node-l act))
                             (actRemove fn (node-r act))))]))
(define (remove-debtors act)
  (local [(define (debtor? act)
            (negative? (node-bal act)))]
    (actRemove debtor? act)))
